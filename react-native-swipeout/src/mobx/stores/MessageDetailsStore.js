import { jpg } from 'Hairfolio/src/helpers';
...
class MessageDetailsStore {
  ...
  async sendVideo(response) {
    ...
    msg.picture.videoFolderName = 'message';
    // send to cloudinary
    let res = await msg.picture.uploadVideo();

    pic = { uri: jpg(res.video_url.url) };
    ...
    msg.picture.videoUrl = res.video_url.url;
  ...
  async sendPicture(response) {
    ...
    msg.picture.folderName = "message"
    // send to cloudinary
