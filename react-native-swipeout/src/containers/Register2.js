...
export default class Register2 extends PureComponent {
  ...
  showExistingUserDialog(isFromInsta = false){
    ...
    LoginManager.logOut();
    UserStore.setHasSessionExpiredWithInsta(false);
...
signUpwithFaceBookFun(token, type, refCode) {
        ...
        Alert.alert(
          ...
          [
            { text: "OK", onPress: () =>{ 
              ...
              App.restartLoginSessionApplicationFB();
            } 
          },
  ...
  _loginWithFacebook = item => {
    ...
    EnvironmentStore.loadEnv()
      ...
      .then(token => {
        showLog("token==>" + token);
        UserStore.facebookToken = token;
        showLog("USER FACEBOOK TOKEN ==> "+UserStore.facebookToken)
        UserStore.checkUserExistence(token, "facebook_token").then(
          res => {
