...
export default class Register2 extends PureComponent {
    ...
    reaction(
      ...
      () => {
        if (OAuthStore.status === READY) {
          ...
          if(OAuthStore.isFromInstaLogin == true)
          {
            this.signUpwithInstagramFun("")
          }
          else
          {
             UserStore.checkUserExistence(OAuthStore.token, "instagram_token")
              .then((res) => {
  ...
  showRefDialogAndValidateCode() {
    ...
    return new Promise((resolve,reject)=>{
      ...
      UserStore.showReferralCodeDialog().then((refCode) => {

        if (refCode == "") {
          resolve(true);
        }
        else {
          UserStore.checkReferralCodeExistence(refCode).then((res) => {
  ...
  _doneTapped = (item) => {
    ...
    if (item && UserStore.registrationMethod === 'email') {
      ...
      this.showRefDialogAndValidateCode().then((res) => {
        ...
        {
          UserStore.showRefConfirmation().then((confirm)=>{
  ...
  signUpwithInstagramFun(refCode){

    UserStore.signupWithInstagram(OAuthStore.token, OAuthStore.userType,refCode)
    .then((res) => {
      showLog("STORE 123==>" + OAuthStore.token + " TYPE ==>" + OAuthStore.userType);
      showLog("signupWithInstagram ==>"+JSON.stringify(res))
      ...
      ServiceBackend.put('users/' + userId, post_data).then(
        (response) => {
          showLog("signupWithInstagram response==>" + JSON.stringify(response))
    ...
    EnvironmentStore.loadEnv()
      ...
      .then(token => {
        showLog("token==>" + token)
           ...
           .then((res)=>
            {
              showLog("LOGIN WITH FACEBOOK USER EXISTENCE ==>" + res)
              if(!res)
              {
                // alert("True Response ==> "+res)
                this.showSocialRefDialogAndValidateCode(token,type,true)
              }
              else
              {
                // alert("False Response ==> "+res)
                this.showExistingUserDialog()
