...
import { ActivityIndicator,Modal, View,TouchableWithoutFeedback,ListView, Text, Image, Dimensions, TouchableOpacity, StyleSheet, ScrollView, TextInput, Keyboard,SafeAreaView,Animated,Easing } from 'react-native';
...
export default class ClientDetails extends Component {
  ...
  { 
    ...
    else if(i==0)
    {
      if(card.expand == true)
      {
        if(temp[1].expand == true || temp[2].expand == true )
        {
          temp[1].expand = false
          temp[2].expand = false
    ...
    return (
      ...
      onLayout={({ nativeEvent }) => {
        ...
        if (title == 'Take Photo') {
          ...
          // showLog('client notes=>'+JSON.stringify(this.state.cards))
          //temporary commentted
          this.captureImage();
        }
  ...
  async onClickObject(item, index) {
    ...
    // CreateLogStore.selectedPictures.push(item);
    // this.props.navigator.push({
  ...
  captureImage() {
    ImagePicker.openCamera({
      ...
      ContactDetailsStore.isScreenPop = false;
      CreateLogStore.selectedPictures.push(image);
      this.props.navigator.push({
        screen: 'hairfolio.CreateLogScreen',
        navigatorStyle: NavigatorStyles.tab,
      });
  ...
  render() {
    ...
    return (
        ...
        <View style={styles.contain}> 
               <Text style={styles.heading}>HairFolio</Text>
          <View style={{backgroundColor:'red',height:200}}> 
             {this.state.cards.map((card, i) => {
                return(
    
                // <Animatable.View  animation = {(card.expand==false )? null : "slideInUp"} duration={500}
                //  style={[styles.card, 
                //   { backgroundColor: card.color,
                //     marginTop: (card.expand == false) ? 
                //                             (i == 0) ? windowHeight/2 : (i==1) ? -10 : -5
                //                                 : 
                //                                 (i == 0) ? windowHeight/6 : (i == 1) ? -250 : -250  ,
                ...
                //     >
                  ...
                  onTouchStart={(e) => {
                    ...
                    {/* <TouchableOpacity style={{height:(card.expand == false) ? collapsedCard : expandedCard,width:'100%'}}
                    onPress={() => { this.expand_collapse_Function(card,i) }}> */}

                    <Text style={styles.textView}>{card.name}</Text>
        ...
        <View style={{ flex: 1 }}>
            ...
            <View>
                {this.state.cards.map((card, i) => {
                  return (
                      <Animatable.View 
                        animation={(card.expand == false) ? null : "slideInUp"}
                        // animation={(card.expand == true) ? 'slideInUp' : "slideInDown"}
                    ...
                    >
                      <TouchableOpacity style={{height:(card.expand == false) ? collapsedCard : expandedCard,width:windowWidth}}
                        onPress={() => { this.expand_collapse_Function(card,i) }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {/* <View style={{  flex: 1,position:'absolute',zIndex:1  }}> */}
              {/* relative ||absolute*/}
            <View style={{ height: (windowHeight-this.state.noteCardHeight), position: this.state.positionStyle }} >
            {/* <View> */}
                    ...
                    // <View style={{backgroundColor:'yellow'}}>
                      <Animatable.View 
                        ...
                        style={[styles.card,
                        {
                          ...
                          backgroundColor: card.color,
                          // marginTop: (card.expand == false) ?
                          //   (i == 0) ? windowHeight / 3.25 : -(windowHeight / 5.5)
                          //   :
                          //   (i == 0) ? windowHeight / 150 : -(windowHeight / 2),
                          marginTop: (card.expand == false) ?
                            // (i == 0) ? windowHeight / 3.25 : -(windowHeight / 5.5)
                            (i == 0) ? 10 : -(windowHeight / 5.5)
                            // (i == 0) ? ((store.addressStreet1+store.addressCity+store.addressState+store.addressPostCode).length>30)?10:-8 : -(windowHeight / 5.5)
                            :
                            (i == 0) ? windowHeight / 150 : -(windowHeight / 2),
      ...
      return (
        <View style={{ flex: 1 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {/* <View style={{ height: (windowHeight / 2), position: this.state.positionStyle }} > */}
            <View style={{ height: (windowHeight- this.state.noteCardHeight), position: this.state.positionStyle,backgroundColor: COLORS.DARK }}>
