...
export default class Feed extends PureComponent {
  ...
  linkingRoute() {
    ...
    Linking.addEventListener("url", event => {
      ...
      setTimeout(()=>{ this.navigateToDetail(event.url); },200);
    }); // added listener
        ...
        // FeedStore.isQrLoading = false;
                ...
                if (o1) {
                  ...
                  internetCheck.testMethod(o => {
                    // FeedStore.isQrLoading = false;
                    if (o == true) {                     
                      setTimeout(()=>{ this.navigateToDetail(o1); },200);
                    } else {
                      FeedStore.isQrLoading = false;
                      setTimeout(() => {
      ...
      internetCheck.testMethod(o => {
        ...
        if (o == true) {
          ...
          cache_id = id; // stored in a global var
            setTimeout(()=>{ this.callSinglePostApi(routeName, id); },200);
    ...
    ServiceBackend.getPostDetail(`posts/${post_id}`).then(
      postDetails => {        
        ...
        if (postDetails.status == 404 || postDetails.status == "404") {
          ...
          setTimeout(()=>{ this.goToPostDetail(postDetails, uniqueCode); },200);         
        }
      },
      error => {
        FeedStore.isQrLoading = false;
        if (count < 3) {
          ...
          setTimeout(()=>{ this.callSinglePostApi(name_route, cache_id); },200);                 
        } else {
          count = 0;
