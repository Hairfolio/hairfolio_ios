import { Alert, convertFraction, ozConv, _ } from 'Hairfolio/src/helpers';
...
class AddServiceStore {
  ...
  async loadColors() {
    ...
    // showLog("LOAD COLOURS ==>"+JSON.stringify(res))
  ...
  @computed get canGoNext() {
    showLog("this.colorNameSelector.hasValue  ==>" + this.colorNameSelector.hasValue);
    showLog("this.serviceSelector.selectedData  ==>" + this.serviceSelector.selectedData)
    // showLog("this.serviceSelector.selectedData.brandCount  ==>"+this.serviceSelector.selectedData.brandCount )
  ...
  @action selectorValueChanged(selector) {
    ...
    if (selector == this.serviceSelector) {
      ...
      ServiceBackend.getBrands(serviceID).then(
        res => {
      ...
      try {
        ...
        ServiceBackend.getLines(brandID)
          .then(res => {
