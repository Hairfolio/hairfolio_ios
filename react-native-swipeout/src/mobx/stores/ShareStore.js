...
import { AlertIOS, AsyncStorage, Clipboard, Image, NativeModules, Share } from 'react-native';
import { AccessToken, LoginManager, ShareApi } from 'react-native-fbsdk';
import ShareA from 'react-native-share';
import { v4 } from 'uuid';
...
class HairfolioStore {
  ...
  async load() {
    ...
    showLog("ShareStore ==>"+JSON.stringify(results));
...
class ShareStore {
  ...
  shareLink(post_id)
  {
    let shareOptions = {
      title: "Hairfolio",
      // message: "Download App from below link https://www.ysparkusa.com/media/catalog/product/cache/4/small_image/254x254/9df78eab33525d08d6e5fb8d27136e95/y/s/ys_park_brush_g_66gwo.jpg",
      url:BASE_URL+`view_post_meta/${post_id}`,
      // url:src,
  ...
  shareReferralCode(code)
  {
    let shareOptions = {
      title: "Hairfolio",
      message: 'Refer this code : ' + code + ' to your friend and receive coupon on your next purchase.Link: http://54.183.167.119:8000/home',
      // url:src,
      subject: "Hairfolio" //  for email
    };
    // ShareA.open(shareOptions);
  ...
  shareReferralCode2(code)
  {
    let shareOptions = {
      title: "Hairfolio",
      message: 'Refer this code : ' + code + ' to your friend and receive coupon on your next purchase.',
      // url:src,
      subject: "Hairfolio" //  for email
    };
    // ShareA.open(shareOptions);
