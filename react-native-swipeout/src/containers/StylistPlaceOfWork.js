...
export default class StylistPlaceOfWork extends React.Component {
    ...
    if (event.type == 'NavBarButtonPress') {
      ...
      if (event.id == 'done') {
        ...
        if (this.state.selected) {          
          ...
          showLog("state here ==>"+JSON.stringify(this.state.selected))


          if(currentUser.account_type == "owner"){
            let temp = this.state.selected;
            temp.id = currentUser.salon.id,            
            formData = { salon_attributes: temp };
          }else{

            if(currentUser.workplace){            
              let temp = this.state.selected;
              temp.id = currentUser.workplace.id,
              temp.salon_id = currentUser.workplace.salon_id
  
              formData = { workplace_attributes: temp };
        
            }else{
              formData = { workplace_attributes: this.state.selected  };
            }
        ...
        else
        {
          ...
          currentUser1 = UserStore.user;

          showLog("currentUser data ==> "+JSON.stringify(currentUser1))
          
          let temp = this.getFormValue();
          showLog("FORM SALON DATA ==> "+JSON.stringify(temp));

          if(currentUser1.account_type == "owner"){
            tempNew = {
              id: currentUser1.salon.id,
              name: temp.name,
            ...
            formData = { salon_attributes: tempNew };
          }else{

            if (currentUser1.workplace) {

              tempNew = {
                id: currentUser1.workplace.id,
                salon_id: currentUser1.workplace.salon_id,
                name: temp.name,
                address: temp.address,
                city: temp.city,
                state: temp.state,
            ...
            formData = { workplace_attributes: tempNew };
          }


          showLog("WORKPLACE FORM DATA ==> " + JSON.stringify(formData));

          UserStore.editUser(formData, UserStore.user.account_type)
            .then((res) => {
              showLog("WORKPLACE UPDATED SUCCESSFULLY ==> " + JSON.stringify(res));
            })
            .catch((e) => {
              this.refs.ebc.error(e);
            });
  ...
  setValue(value = {}) {
    ...
    this.setFormValue({
      ...value,
      'salon_id': value.salon_id || -1
    });
    this.setState({
      selected: value.salon_id
    });
  }
  ...
  loadForm = () => {
    ...
    ServiceBackend.get(`users/${UserStore.user.id}`)
      .then((response) => {
        ...
        if (user.workplace) {
          this.setFormValue({
            id:user.workplace.id,
            name: user.workplace.name,
            'salon_id': user.workplace.id,
            address: user.workplace.address,
            city: user.workplace.city,
            state: user.workplace.state,
            zip: user.workplace.zip,
            website: user.workplace.website,
            phone: user.workplace.phone,
          });
          var temp = {
            id:user.workplace.id,
            name: user.workplace.name,
    ...
    return (
      <BannerErrorContainer ref="ebc" style={{
        ...
        >
          ...
          <HiddenInput
            ref={(r) => this.addFormItem(r, 'salon_id')}
          />
