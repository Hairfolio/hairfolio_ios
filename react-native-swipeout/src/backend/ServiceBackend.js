...
class ServiceBackend extends Backend {
  ...
  async pinHairfolio(hairfolio, post) {
    ...
    let res = await this.put('folios/' + hairfolio.id, {
      folio: {
        name: hairfolio.name
      }
    });

    let pinRes = await this.post(`folios/${hairfolio.id}/add_post`, {
      post_id: post.id
    });
  ...
  async getEnvironment() {
    return this.fetchEnvironment('sessions/environment');
  }
}
